...
t_unix_char_limit g_t_global_limit[] = {
...
int			ft_unix_string_display(wchar_t *str)
{
	...
	for (i = 0; i < 0x10FFFF; i++)
	{
		...
		else
		{
			...
			{
				if (ft_unix_char_is_valid(i))
				{
					ft_unix_string_display_ii(i);
					write(1, &e, 1);	
				}
			}
			if (ft_unix_char_is_display_iii(i) == 1)
			{
				if (ft_unix_char_is_valid(i))
				{
					ft_unix_string_display_iii(i);
					write(1, &e, 1);	
				}
			}
			if (ft_unix_char_is_display_iv(i) == 1)
			{
				if (ft_unix_char_is_valid(i))
				{
					ft_unix_string_display_iv(i);
					write(1, &e, 1);	
				}
			}
		}
